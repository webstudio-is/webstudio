...
Reference a uniquely named top-level custom template with capitalized JSX such as `<PromotionCard tone="featured">Content</PromotionCard>`. Names must be valid JSX component identifiers, attributes must be static, and explicit children replace the template root's default children. A self-closing reference such as `<PromotionCard />` keeps its designed default subtree. Imports, expressions, spreads, functions, lowercase HTML-looking JSX, and executable JavaScript are unsupported. Use the legacy `<ws.element ws:name="Promotion Card">` form when the template name is not JSX-compatible. Preserve unresolved template names and report their diagnostics.

When a template is renamed or deleted, use `migrate-content-block-template-references` to update the affected MDX files. Its first call returns a plan with changed-file, update, omission, and diagnostic counts. Report the plan and repeat the exact request with its `confirmationToken` only after approval. Renames and removals update both JSX and legacy `ws:name` references. Renaming JSX to a name that is not JSX-compatible converts that reference to the legacy form. Invalid files remain unchanged and are reported in diagnostics.
