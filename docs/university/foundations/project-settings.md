...
Use **Project settings > Headers** to set or remove HTTP response headers for your site.

On Webstudio Cloud, publishing custom headers to a custom domain requires Pro. Staging is free.

1. Enter a header name, such as `Content-Security-Policy`.
2. Enter its value, or select **Remove header** to remove it from the response.
3. Click **Add header** and publish the site.

An empty value sends the header with no value. **Remove header** removes the header, while **Delete rule** stops overriding it. Publish again after changing a rule.

These are standard HTTP headers. Webstudio Cloud applies the rules automatically to site responses. On other hosts, your server or hosting adapter must apply the configuration generated by the CLI in `app/__generated__/$resources.headers.server.ts`. Any proxy or CDN must preserve the resulting headers. Other hosting templates and static exports do not apply these rules automatically.
...
On Webstudio Cloud, the rules also apply to redirects and site errors. Static assets served by the CDN keep their existing headers.
