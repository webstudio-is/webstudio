...
Use **Project settings > Headers** to configure your site's HTTP security response headers. Each header starts with a default value:

| Header | Default | Required |
| --- | --- | --- |
...
To allow embedding by `https://example.com`, change Content-Security-Policy to `frame-ancestors 'self' https://example.com` and remove X-Frame-Options.

These are standard HTTP headers. Webstudio Cloud applies the settings automatically to site responses, including redirects and site errors. Static assets served by the CDN keep their existing headers. On other hosts, your server or hosting adapter must apply the configuration generated by the CLI in `app/__generated__/$resources.headers.server.ts`, and any proxy or CDN must preserve the resulting headers. Other hosting templates and static exports do not apply these settings automatically.

Header names are case-insensitive. Each header can be configured once, with values up to 8 KB and a combined limit of 16 KB. Values cannot contain line breaks. Arbitrary headers, including cache directives and cookies, cannot be configured in these settings.
