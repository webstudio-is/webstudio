...
import {
  createFsClient,
  createFsImmutableResourceIndexStore,
  createS3Client,
  createS3ImmutableResourceIndexStore,
...
export const createAssetResourceIndexStore = () => {
  if (
    env.S3_ENDPOINT !== undefined &&
    env.S3_REGION !== undefined &&
    env.S3_ACCESS_KEY_ID !== undefined &&
    ...
    return createS3ImmutableResourceIndexStore({
      endpoint: env.S3_ENDPOINT,
      region: env.S3_REGION,
      accessKeyId: env.S3_ACCESS_KEY_ID,
      secretAccessKey: env.S3_SECRET_ACCESS_KEY,
  ...
  return createFsImmutableResourceIndexStore(
    path.join(process.cwd(), fileUploadPath)
  );
};
