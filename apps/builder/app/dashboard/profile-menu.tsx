      ...
      <DropdownMenuContent align="start" width="regular">
        ...
        {purchases.map((purchase, index) => (
          <DropdownMenuItem
            key={index}
            onSelect={() => {
              if (purchase.manageUrl) {
                // New path: worker provides the manage URL
                const url = new URL(purchase.manageUrl, window.location.origin);
                url.searchParams.set("return_url", window.location.href);
                navigate(url.pathname + url.search);
              } else if (purchase.subscriptionId) {
                // Old path: n8n billing portal
