...
Webstudio projects store manually editable route authentication config at:
...
Build tools merge auth sources in this order:

1. Existing `.webstudio/auth.json`
2. Project-level auth config from Webstudio project settings
...
duplicates. Because routes are JSON object keys, a single config file cannot
represent duplicate routes.
