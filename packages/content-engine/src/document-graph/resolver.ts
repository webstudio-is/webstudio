...
export const resolveDocumentGraph = async <Source, Value>({
  ...
  for (const node of closure) {
    ...
    for (const edge of edgesBySourceId.get(node.id) ?? []) {
      ...
      if (
        ...
        values.has(edge.reference.documentId) === false
      ) {
        throw new DocumentGraphResolutionError({
      ...
      const targetValue = values.get(edge.reference.documentId) as Value;
    ...
    try {
      values.set(node.id, await assemble({ node, source, references }));
    } catch (error) {
      throw new DocumentGraphResolutionError({
        code: "DOCUMENT_ASSEMBLY_FAILED",
  ...
  return Object.freeze({
    roots: Object.freeze(rootIds.map((rootId) => values.get(rootId) as Value)),
    values,
  });
};
