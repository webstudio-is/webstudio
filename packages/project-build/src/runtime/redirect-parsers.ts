...
import { getRedirectSourceSearchIndex } from "./redirect-source";
import {
...
const normalizeRecord = (
  ...
  const sourceErrors = validateRedirectSource({
  ...
  if (sourceErrors.length > 0) {
    return {
      skipped: {
        line: lineNumber,
        content: originalContent,
        reason: sourceErrors.join(", "),
      },
    };
  }

  const targetErrors = validateRedirectTarget({
    target: normalizedTo,
    source: normalizedFrom,
  });
