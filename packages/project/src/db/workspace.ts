...
const publishesPerWorkspaceSeat = 100;
...
export const getWorkspaceDailyPublishLimit = (
  ...
  const workspaceSeats = 1 + plan.seatsIncluded + (extraSeats ?? 0);
  return workspaceSeats * publishesPerWorkspaceSeat;
};
...
export const getWorkspacePublishUsage = async (
  ...
  if (project.data.userId === null) {
    throw new Error("Project must have project userId defined");
  }

  const ownerId = project.data.userId;
  const [plan, extraSeats] = await Promise.all([
    getPlanFeaturesByOwnerId(ownerId, context),
    getExtraPaidSeats(ownerId, context),
  ]);
  const limit = getWorkspaceDailyPublishLimit(plan, extraSeats);

  if (project.data.workspaceId === null) {
    ...
    return { count: result.data?.count ?? 0, limit };
  }
  ...
  const result = await context.postgrest.client
    .from("Build")
    .select("id, Project!inner(workspaceId)", { count: "exact", head: true })
    .eq("Project.workspaceId", project.data.workspaceId)
...
export const assertWorkspacePublishAllowed = async (
  projectId: string,
  context: AppContext
) => {
  const { count, limit } = await getWorkspacePublishUsage(projectId, context);
  if (count >= limit) {
    throw new Error(
      `This workspace has reached its daily publishing limit of ${limit}. The limit resets tomorrow.`
    );
  }
};
