...
import { cliDiff } from "./prisma-command";
import { migrationsDir, schemaFilePath } from "./prisma-paths";

const templateFilePath = path.join(migrationsDir, "template.txt");
const lockfilePath = path.join(migrationsDir, "lockfile");
...
${schemaContent}`;
  ...
  const migrationSchemaFilePath = path.join(
    migrationsDir,
    migrationName,
    "schema.prisma"
  );
  writeFile(migrationSchemaFilePath, schemaContent);
  logger.info(`Created: ${migrationSchemaFilePath}`);

  await prismaMigrations.generateMigrationClient(migrationName);
  logger.info(`Created: ${path.join(migrationsDir, migrationName, "client")}`);
