...
For Codex, `connect` prints the explicit `codex mcp add` command instead of
silently editing global configuration. Before changing client configuration,
`connect` verifies that the saved project endpoint is reachable and its
credential is accepted. If verification fails, relink with a current editable
share link and retry.
