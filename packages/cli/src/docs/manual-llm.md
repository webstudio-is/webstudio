...
- Use a metadata-only Assets resource with `content.mode:"none"` for the `/blog` listing. Combine filters with `where.all` (AND) and `where.any` (OR). Give both blog queries static Markdown and folder/path constraints, including `extension = md`, so dynamic route compilation cannot pull images or unrelated files into the database. Use a separate Assets resource for `/blog/:slug` with those same static constraints plus a slug or alternate-ID filter whose value is `system.params.slug`, `limit:1`, and `content.mode:"markdown-body"`. Request `full` content only when the whole source file is rendered; otherwise prefer `markdown-body` or a bounded byte range.
