...
4. Use focused MCP calls with concrete JSON: `webstudio meta.guide '{"brief":"Create a design system page using every component"}'`, `webstudio meta.get_more_tools '{"tools":["insert-fragment"]}'`, `webstudio components.coverage-plan`, `webstudio components.find '{"brief":"radix select"}'`, and `webstudio components.get '{"component":"Box"}'`.
...
1. Discover capabilities with `webstudio man --json`, `webstudio schema api`, `webstudio schema mcp`, MCP `meta.index`, `meta.guide`, `meta.get_more_tools`, `components.summary`, `components.coverage-plan`, `components.find`, and `components.get`. From a shell, prefer shortcut calls such as `webstudio meta.index` and `webstudio components.find '{"brief":"button"}'` for these focused tool calls; use `webstudio mcp single-op-call` when you need the explicit MCP form. Read full resources such as `webstudio://project/tools` and `webstudio://project/components` only when needed. Do not write scripts to parse full MCP discovery JSON for normal lookup.
