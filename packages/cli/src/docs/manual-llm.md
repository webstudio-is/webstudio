...
- Page metadata fields such as `title`, `description`, `language`, `redirect`, and custom meta content accept plain fixed text. For computed values, pass JavaScript expression code such as `pageTitle ?? "Pricing | Acme"`. When an expression could also be read as fixed text, pass an explicit value such as `{"expression":"first - second"}`.
