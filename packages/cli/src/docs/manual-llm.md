...
- Do **not** embed a Markdown body in an Assets query unless it is absolutely necessary. Prefer a small JSON descriptor for each renderable document, for example `{ "kind":"post", "slug":"hello", "body":{ "$ref":"./hello.md#body" } }`. Query the descriptors with static constraints such as `extension = json` and `properties.kind = post`, select `properties.body`, and keep `content.mode:"none"`. The document graph fetches the referenced Markdown from Asset storage and resolves only the selected result at runtime.
- Use direct content modes only for legacy or read-only source sets where a descriptor cannot be authored, or when the caller explicitly requires embedded bytes. Even then, filter to the intended file first and use `markdown-body`, `range`, or `full` in that preference order. Never choose `markdown-body` merely because the final component is Markdown Embed.
- Markdown selected with `#body` excludes frontmatter. References and Markdown returned through the legacy `content.mode:"markdown-body"` path both resolve conventional relative links and images such as `../images/hero.png` to matching Assets. Markdown Embed permits sanitized figures, audio, video, and iframes, but removes scripts, inline event handlers, and unsafe URLs.
- Assets expose an ID-keyed map at `<dataSourceName>.data` and collection information at `<dataSourceName>.meta`. Bind a listing Collection to `posts.data` and a one-result detail Collection to `post.data`; each item value contains selected fields and its item key is the asset ID. Read frontmatter or JSON fields from `item.properties`. Bind referenced Markdown from a descriptor to `item.properties.body`. The legacy embedded-content path is available at `item.content.text` only when a content mode was explicitly necessary.
- Use `preview-asset-query` with concrete values before binding expressions in the saved resource. Inspect `__diagnostics__.query` for the temporary query-only footprint and `__diagnostics__.database` for the merged database built from all reachable Assets queries. Only `database.usedBytes` counts toward `database.maxBytes`; query sizes are not separate allowances and must not be summed. Compare `usedBytes`, `unboundedBytes`, and `truncated` within both scopes. When merged usage approaches the limit, first remove unused output fields, then narrow candidate files with filters, and finally reduce or remove embedded content. Inspect saved mode and configuration with `list-assets-resources` or `get-assets-resource`; shared index maintenance is automatic.
