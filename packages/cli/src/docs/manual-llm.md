...
- Use a metadata-only Assets resource with `content.mode:"none"` for listings. Use a separate Assets resource for the detail route with a slug filter whose value is `system.params.slug`, `limit:1`, and `content.mode:"markdown-body"`.
