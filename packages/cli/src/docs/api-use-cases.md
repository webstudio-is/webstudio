...
- Read the Content Block's Templates children before writing a custom reference. Use capitalized JSX such as `<PromotionCard />` for a unique JSX-compatible top-level instance name, or legacy `<ws.element ws:name="Promotion Card">` syntax for other names. JSX attributes must be static, explicit children replace defaults, and self-closing references keep the designed default subtree. Preserve unresolved names and report their diagnostics instead of deleting them.
