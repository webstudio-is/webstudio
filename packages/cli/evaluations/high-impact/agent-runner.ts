...
export const createMinimalAgentTask = (
  ...
  return {
    ...
    constraints: [
      ...
      ...(fixture.id === "markdown-blog-v1"
        ? [
            'For this fixture, call meta.get_more_tools exactly once immediately after meta.guide with {"brief":"create-assets-resource"} to load the complete schema for its complex query input. Do not call meta.index, list/get/preview asset tools, list pages, or any other discovery operation. Then create one asset folder named Blog and upload all supplied Markdown files, JSON post descriptors, and northstar-author.json together with exactly one upload-assets call using type file, each file format shown in the exact input, that returned folder id, empty metadata, and assetsDir .webstudio/assets.',
            ...
            'First call create-assets-resource exactly once with this complete overview input, substituting the returned overview root id and Blog folder id: {"name":"Published posts","scopeInstanceId":"<overview-root-id>","dataSourceName":"posts","query":{"where":{"all":[{"field":["extension"],"operator":"eq","value":{"type":"literal","value":"json"}},{"field":["properties","kind"],"operator":"eq","value":{"type":"literal","value":"post"}},{"field":["folderId"],"operator":"eq","value":{"type":"literal","value":"<blog-folder-id>"}},{"field":["properties","draft"],"operator":"ne","value":"true"}]},"sort":[{"field":["properties","publishedAt"],"direction":"desc"},{"field":["id"],"direction":"asc"}],"limit":"20","output":{"mode":"fields","includeMetadata":false,"fields":[["properties","title"],["properties","slug"],["properties","publishedAt"],["properties","author"],["properties","excerpt"]]},"content":{"mode":"none"}}}. Do not convert expression strings to booleans or field paths to dotted strings.',
            'Then call create-assets-resource exactly once with this complete detail input, substituting the returned detail root id and the same Blog folder id: {"name":"Post by slug","scopeInstanceId":"<detail-root-id>","dataSourceName":"post","query":{"where":{"all":[{"field":["extension"],"operator":"eq","value":{"type":"literal","value":"json"}},{"field":["properties","kind"],"operator":"eq","value":{"type":"literal","value":"post"}},{"field":["folderId"],"operator":"eq","value":{"type":"literal","value":"<blog-folder-id>"}},{"field":["properties","slug"],"operator":"eq","value":"system.params.slug"}]},"limit":"1","output":{"mode":"fields","includeMetadata":false,"fields":[["properties","title"],["properties","author"],["properties","body"]]},"content":{"mode":"none"}}}. Do not call create-assets-resource again.',
