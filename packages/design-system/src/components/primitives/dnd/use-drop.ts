  ...
  return useMemo(() => {
    ...
    const setDropTarget = (partialDropTarget: PartialDropTarget<Data>) => {
      ...
      if (
        ...
        let placement = toGlobalCoordinates(
        ...
        const propsChildrenOrientation =
          latestProps.current.childrenOrientation;
        if (
          propsChildrenOrientation !== undefined &&
          placement !== undefined &&
          placement.direction === propsChildrenOrientation.type
        ) {
          placement = undefined;
        }

        if (placement === undefined) {
          placement = toGlobalCoordinates(
            getPlacementNextTo(
              parentRect,
              closestChildRect,
              childrenOrientation,
